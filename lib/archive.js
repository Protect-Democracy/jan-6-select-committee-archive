 ...
 * @param {*} date
 */
async function archive(destination, date, overwrite, pageId = null) {
  ...
  const archiveInventory = {
    [date]: {
      ...
      inventory_csv: `${archiveUrlBase}/${pathJoin(date, "inventory.csv")}`,
      archive_file: `${archiveUrlBase}/${pathJoin(date, `${date}.zip`)}`,
      archive_site: `${archiveUrlBase}/${pathJoin(
        date,
        `${date}-january6th.house.gov.zip`
      )}`,
      archive_site_devindc: `${archiveUrlBase}/${pathJoin(
        date,
        `${date}-house.devindc.com.zip`
      )}`,
