...
async function preserveMaterial(
  ...
  if (
    ...
    material.material_type !== "stream-video" &&
    material.material_type !== "screenshot"
  ) {
    ...
    const headers = Object.entries(commonHeaders)
      ...
      .join(" ");
    const command = `${bin} --add-header "user-agent:${userAgentBrowserGenerate()}" ${headers} --output "${videoFileName}" -f "${format}" "${
